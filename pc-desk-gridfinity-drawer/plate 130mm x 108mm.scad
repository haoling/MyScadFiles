...
module plate130x108() {
...

  {
    ...
    translate([108, 0, 0])
    {
      ...
      translate([0, space + (PITCH * 1), 0]) rail(pillar = false, sideL = false, sideR = ! standalone);
      ...
      translate([0, space + (PITCH * 4), 0]) rail(pillar = false, sideL = false, sideR = ! standalone);
      translate([0, space + (PITCH * 5), 0]) rail(pillar = true, sideL = false, sideR = false);
      translate([0, space + (PITCH * 6), 0]) rail(length = space);
    }

    // 手前側エッジ
    if (standalone) translate([-1, 0, 0]) plate_edge(width = space + PITCH, slope = true, joint = false);
    ...
    for (y = [1 : 4])
    {
      if (standalone) translate([-1, space + (y * PITCH), 0]) plate_edge();
      for (x = [0 : 5])
      {
    ...
    // 奥側エッジ
    if (standalone) translate([-1, space + (5 * PITCH), 0]) plate_edge(rear = true, width = space + PITCH, slope = true, joint = false);
